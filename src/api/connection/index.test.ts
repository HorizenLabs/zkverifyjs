...
describe('establishConnection', () => {
    ...
    beforeEach(() => {
        mockApiPromiseCreate = ApiPromise.create as jest.MockedFunction<typeof ApiPromise.create>;
        mockWsProvider = new WsProvider('ws://localhost') as jest.Mocked<WsProvider>;
        mockWaitForNodeToSync = waitForNodeToSync as jest.MockedFunction<typeof waitForNodeToSync>;
    ...
    it('should establish a connection successfully on a custom network', async () => {
        ...
        const result = await establishConnection(SupportedNetwork.Custom, customUrl);
    ...
    it('should throw an error if a custom WebSocket URL is provided but the host is not custom', async () => {
        await expect(establishConnection(SupportedNetwork.Testnet, 'ws://custom-url')).rejects.toThrow(
    ...
    it('should throw an error if ApiPromise.create fails', async () => {
        mockApiPromiseCreate.mockRejectedValueOnce(new Error('API creation failed'));
