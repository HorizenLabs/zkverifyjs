...
describe('registerVk', () => {
    ...
    beforeEach(() => {
        ...
        (connection.api.tx[mockPallet].registerVk as unknown as jest.Mock).mockReturnValue(mockExtrinsic);
    });
    ...
    it('should successfully register verification key and handle transaction', async () => {
        ...
        const { events, transactionResult } = await registerVk(connection, mockOptions, mockVerificationKey);
        ...
        expect(connection.api.tx[mockPallet].registerVk).toHaveBeenCalledWith('formattedVk');
    ...
    it('should throw an error for unsupported proof type', async () => {
        ...
        await expect(registerVk(connection, mockOptions, mockVerificationKey)).rejects.toThrow(
            `Unsupported proof type: ${mockOptions.proofType}`,
        );
    });
