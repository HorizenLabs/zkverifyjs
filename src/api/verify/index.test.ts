...
describe('verify', () => {
  ...
  it('should throw an error if proof or publicSignals are missing', async () => {
    ...
    await expect(
      ...
      `${mockOptions.proofType}: Proof and publicSignals are required and cannot be null or undefined.`,
    );
  ...
  it('should throw an error if formatting verification key fails', async () => {
    ...
    await expect(
      ...
      `Failed to format ${mockOptions.proofType} verification key: Formatting error. Verification key snippet: "vk..."`,
    );
  });
