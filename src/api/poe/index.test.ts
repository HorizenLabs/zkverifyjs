...
const mockLeaf =
  '0x85bb9bbddf1eae951835add9e138087dcea9fc6bf156b065b3232edeb3394d5e';
const mockNumberOfLeaves = 10;
const mockLeafIndex = 3;

const createH256 = (value: string) =>
  createTypeUnsafe(registry, 'H256', [value]) as H256;
...
describe('getProofDetails', () => {
  ...
  it('should return proof details when blockHash is not provided', async () => {
    ...
    expect(api.rpc.poe.proofPath).toHaveBeenCalledWith(
      attestationId,
      leafDigest,
    );
  ...
  it('should return proof details when blockHash is provided', async () => {
    ...
    const result = await getProofDetails(
      api,
      attestationId,
      leafDigest,
      blockHash,
  ...
  it('should throw an error if the API call fails', async () => {
    // @ts-expect-error: Custom RPC method 'poe.proofPath' is not recognized by TypeScript's type system
    (api.rpc.poe.proofPath as jest.Mock).mockRejectedValue(
      new Error('API call failed'),
    );

    await expect(
      getProofDetails(api, attestationId, leafDigest),
    ).rejects.toThrow('Failed to retrieve proof details: API call failed');
  });
  ...
  it('should throw an error if the blockHash is invalid', async () => {
    ...
    await expect(
      getProofDetails(api, attestationId, leafDigest, 'invalidBlockHash'),
    ).rejects.toThrow('Failed to retrieve proof details: Invalid blockHash');
  });
});
